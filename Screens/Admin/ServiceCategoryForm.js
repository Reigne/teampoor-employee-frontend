...
const ServiceCategoryForm = (props) => {
  ...
  const validateForm = () => {
    ...
    if (!name) errors.name = "Service category name is required";
  ...
  const addCategory = () => {
    ...
    if (item !== null) {
      ...
      axios
        .put(`${baseURL}serviceCategories/${item.id}`, formData, config)
        .then((res) => {
      ...
      axios
        .post(`${baseURL}serviceCategories/`, formData, config)
        .then((res) => {
          if (res.status == 200 || res.status == 201) {
            Toast.show({
              topOffset: 60,
      ...
      <View className="bg-gray  flex-1 ">
        ...
        >
          <Text className="font-bold text-2xl mb-3">
            {!props.route.params ? "Create Sevice Category" : "Update Sevice Category"}
          </Text>
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            {loading ? (
              ...
              <Text className="font-xl font-bold text-center text-white ">
                {!props.route.params ? "Create Service Category" : "Update Service Category"}
              </Text>
            )}
          </View>
        </TouchableOpacity>
